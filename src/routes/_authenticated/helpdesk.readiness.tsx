...
import { getHelpdeskReadiness, type CheckStatus } from "@/lib/helpdesk/readiness.functions";

export const Route = createFileRoute("/_authenticated/helpdesk/readiness")({
  head: () => ({
    meta: [
      { title: "Helpdesk Readiness" },
      { name: "description", content: "Production readiness review across tickets, SLA, escalations, AI, KB, analytics, and integrations." },
    ],
  }),
...
function ReadinessPage() {
  const run = useServerFn(getHelpdeskReadiness);
  const { data, isLoading, isFetching, refetch } = useQuery({
    queryKey: ["helpdesk-readiness"],
    queryFn: () => run({}),
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button asChild variant="ghost" size="sm">
          <Link to="/helpdesk"><ArrowLeft className="mr-1 size-4" />Back</Link>
        </Button>
        <div className="flex-1">
          <h1 className="text-2xl font-semibold">Helpdesk Production Readiness</h1>
          <p className="text-sm text-muted-foreground">
            End-to-end review across tickets, CRM, omnichannel, SLA, escalations, AI, knowledge base, analytics, security and UX.
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => refetch()} disabled={isFetching} aria-label="Re-scan">
        ...
        <>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <Link to="/knowledge" className="rounded-md border p-3 hover:bg-muted">
                <div className="text-sm font-medium">Knowledge Base</div>
                <div className="text-xs text-muted-foreground">RAG suggestions</div>
              </Link>
              <Link to="/automations" className="rounded-md border p-3 hover:bg-muted">
                <div className="text-sm font-medium">Workflows</div>
                <div className="text-xs text-muted-foreground">Escalations & automation</div>
              </Link>
            </CardContent>
          </Card>
        </>

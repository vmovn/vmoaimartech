  ...
  try {
    ...
    const res = await fetch(endpoint.url, {
      ...
      headers: {
        ...
        "user-agent": "Swiffer-Webhooks/1.0",
        "x-swiffer-event": delivery.event_type,
        "x-swiffer-event-id": delivery.event_id,
        "x-swiffer-delivery-id": delivery.id,
        "x-swiffer-signature": sig,
        "x-swiffer-attempt": String(attempt),
        ...(endpoint.headers ?? {}),
      },
      body,
    });

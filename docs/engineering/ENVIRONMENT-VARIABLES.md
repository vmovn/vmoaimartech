...
| `OLLAMA_MAX_CONCURRENCY` | Max concurrent platform-managed Ollama chats in this Node process (default 2, clamp 1–8) | Optional | No | Runtime server | Platform-managed Ollama | Conditional |
| `OLLAMA_WORKSPACE_MAX_CONCURRENCY` | Max concurrent platform-managed Ollama chats per workspace in this process (default 1, clamp 1–4) | Optional | No | Runtime server | Platform-managed Ollama | Conditional |
...
Ollama is a keyless self-hosted provider. Production must set `OLLAMA_BASE_URL` to the operator-controlled internal URL (do not assume localhost or a Coolify hostname in source). Local development may omit it and use `http://localhost:11434/v1`. `OLLAMA_UTILITY_MODEL` selects the workspace utility model used when seeding `conversation_intelligence`; it is not a customer-facing chatbot default. `OLLAMA_MAX_CONCURRENCY` / `OLLAMA_WORKSPACE_MAX_CONCURRENCY` bound in-flight shared Ollama calls per Node process (not a substitute for the per-minute rate limiter). Caps are replica-local.

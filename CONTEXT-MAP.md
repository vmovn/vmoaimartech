...
Verified against GitHub `main` at `v1.0.0-localhost-1.0.5` / `67704b8967b6db5cb2a9389d8f1a7f2f836783ea` on 2026-08-30.

## Task Router

| If the task is about… | Owning context | Read first | Primary starting surface |
|---|---|---|---|
| first-run `/setup`, Setup Secret, Super Admin, setup completion | Setup | `docs/contexts/setup/CONTEXT.md` | `src/routes/setup.tsx`, `src/lib/setup/**` |
| env keys, Coolify, public/server secrets, readiness | Environment | `docs/contexts/environment/CONTEXT.md` | `.env.example`, `scripts/dev/**`, setup readiness/catalog if present |
| login, session, auth guards, authenticated server functions | Auth | `docs/contexts/auth/CONTEXT.md` | `src/lib/auth/**`, auth routes |
| organization, workspace, membership, RBAC, tenant boundaries | Platform | `docs/contexts/platform/CONTEXT.md` | `src/lib/tenant/**`, `src/lib/organization.ts`, `src/lib/workspace*`, RBAC |
...
| leads, contacts, customers, companies, lifecycle, segmentation | CRM | `docs/contexts/crm/CONTEXT.md` | `src/lib/leads.ts`, `contacts.ts`, `companies.ts`, `src/lib/crm/**` |
| deals, pipeline, products, quotes, invoices, activities | Sales | `docs/contexts/sales/CONTEXT.md` | `src/lib/deals.ts`, `products.ts`, `quotes.ts`, `invoices.ts`, `activities.ts` |
...
| AI configuration, providers, completion, customer intelligence | AI | `docs/contexts/ai/CONTEXT.md` | `src/lib/ai/**`, `src/lib/ai-providers/**` |
...
| client/customer self-service portal | Client Portal | `docs/contexts/client-portal/CONTEXT.md` | client-portal routes/services discovered by task |

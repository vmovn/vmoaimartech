...
UPDATE public.plans SET
  tagline = COALESCE(tagline, 'Dành cho người kinh doanh độc lập đang trải nghiệm nền tảng'),
  cta_label = COALESCE(cta_label, 'Bắt đầu miễn phí'),
  sort_order = 10,
...
UPDATE public.plans SET sort_order = 30, cta_label = COALESCE(cta_label, 'Dùng thử miễn phí') WHERE code = 'professional';
UPDATE public.plans SET sort_order = 31 WHERE code = 'professional_yearly';

-- Business
UPDATE public.plans SET
  sort_order = 40,
  cta_label = COALESCE(cta_label, 'Dùng thử miễn phí'),
...
UPDATE public.plans SET
  sort_order = 50,
  cta_label = COALESCE(cta_label, 'Liên hệ kinh doanh'),
  tagline = COALESCE(tagline, 'Hạn mức tùy chỉnh, SSO và hỗ trợ riêng'),
